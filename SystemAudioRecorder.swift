...
class AudioStreamOutput: NSObject, SCStreamOutput {
    ...
    private func logAudioFormat(_ format: AudioStreamBasicDescription) {
        Logger.log("Audio format details:", level: .info)
        Logger.log("- Sample rate: \(format.mSampleRate) Hz", level: .info)
        Logger.log("- Channels: \(format.mChannelsPerFrame)", level: .info)
        Logger.log("- Bits per channel: \(format.mBitsPerChannel)", level: .info)
        Logger.log("- Bytes per frame: \(format.mBytesPerFrame)", level: .info)
        Logger.log("- Format ID: \(format.mFormatID)", level: .info)
        Logger.log("- Format flags: \(format.mFormatFlags)", level: .info)
    }
}
